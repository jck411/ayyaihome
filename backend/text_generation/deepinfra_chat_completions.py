...
from backend.config.clients import get_deepinfra_client

# Initialize a logger for the module
logger = logging.getLogger(__name__)

def extract_content_from_deepinfra_chunk(chunk: Any) -> Optional[str]:
    """
    Extracts the content string from DeepInfra's streaming response.

    Args:
        chunk (Any): The chunk object from DeepInfra's streaming response.

    Returns:
        Optional[str]: The extracted content string or None if not available.

    This function assumes that DeepInfra's API streaming response has a structure where
    content is stored in 'choices[0].delta.content'. If not present, it returns None.
    """
    try:
...
async def process_chunks(
    ...
    while True:
        ...
        content = extract_content_from_deepinfra_chunk(chunk)
        if content:
...
async def stream_deepinfra_completion(
    ...
    """
    Stream DeepInfra completion and process chunks for phrase extraction.

    Args:
        messages (Sequence[Dict[str, Union[str, Any]]]): List of conversation messages to send to DeepInfra.
        phrase_queue (asyncio.Queue): Queue to enqueue extracted phrases for downstream processing.
        client (Optional[AsyncOpenAI]): Optional DeepInfra client for making API requests.

    Returns:
        AsyncIterator[str]: Async iterator of streaming text chunks.

    This function handles DeepInfra's streaming response by:
    1. Sending the conversation messages to DeepInfra's API.
    ...
    client = client or get_deepinfra_client()
    ...
    try:
        # Send the request to DeepInfra's API and begin streaming the response
        response = await client.chat.completions.create(
            model=Config.LLM_CONFIG.DEEPINFRA_RESPONSE_MODEL,
            messages=messages,  
            stream=True,
            temperature=Config.LLM_CONFIG.DEEPINFRA_TEMPERATURE,
            top_p=Config.LLM_CONFIG.DEEPINFRA_TOP_P,
        )
        logger.info("DeepInfra streaming response started.")
        ...
        logger.error(f"Error during DeepInfra streaming: {e}")
        await chunk_queue.put(None)  # Ensure processing task can terminate
        raise HTTPException(status_code=500, detail=f"DeepInfra API error: {e}")
