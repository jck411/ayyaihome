...
from backend.config.clients import get_openrouter_client

# Initialize logger
logger = logging.getLogger(__name__)

# ------------------------------
# Helper Functions for Extraction and Processing
# ------------------------------

def extract_content_from_openrouter_chunk(chunk: Any) -> Optional[str]:
    """
    Extracts the content string from OpenRouter's response chunk.

    Args:
        chunk (Any): The chunk object from OpenRouter's streaming response.

    Returns:
        Optional[str]: The extracted content string or None if not available.

    This function accesses the content string in the response. If the structure
    of the chunk is unexpected, it logs a warning and returns None.
    """
    try:
...
async def process_chunks(
    ...
    while True:
        ...
        content = extract_content_from_openrouter_chunk(chunk)
        if content:
...
async def stream_openrouter_completion(
    ...
    """
    Stream OpenRouter completion and process chunks for phrase extraction.

    Args:
        messages (Sequence[Dict[str, Union[str, Any]]]): List of conversation messages to send to OpenRouter.
        phrase_queue (asyncio.Queue): Queue to enqueue extracted phrases for downstream processing.
        client (Optional[AsyncOpenAI]): Optional OpenRouter client for making API requests.

    Returns:
        AsyncIterator[str]: Async iterator of streaming text chunks.

    This function handles OpenRouter's streaming response by:
    1. Sending the conversation messages to OpenRouter's API.
    ...
    try:
        # Send the request to OpenRouter's API and begin streaming the response
        response = await client.chat.completions.create(
            model=Config.LLM_CONFIG.OPENROUTER_RESPONSE_MODEL,
            messages=[{"role": "system", "content": Config.LLM_CONFIG.OPENROUTER_SYSTEM_PROMPT}] + messages,
            stream=True,
            temperature=Config.LLM_CONFIG.OPENROUTER_TEMPERATURE,
            top_p=Config.LLM_CONFIG.OPENROUTER_TOP_P,
        )
        logger.info("OpenRouter streaming response started.")
