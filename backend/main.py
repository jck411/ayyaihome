...
def text_to_speech_processor(phrase_queue: queue.Queue, audio_queue: queue.Queue):
    logger.info("Text-to-Speech processor started.")
    while not stop_event.is_set():
        phrase = phrase_queue.get()
        if phrase is None:
            audio_queue.put(None)
            logger.info("Text-to-Speech processor stopped.")
            return
        ...
        try:
            ...
            logger.info("Phrase processed and audio data generated.")
...
async def openai_stream(request: Request):
    ...
    logger.info("Received request with messages.")
    ...
    async def generate():
        try:
            ...
            logger.info("Finished streaming response from OpenAI.")
